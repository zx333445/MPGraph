...
class BatchWSI(torch_geometric.data.Batch): # type: ignore
    def __init__(self):
            ...
            for i, data in enumerate(data_list):
                for key in keys:
                    ...
                    if isinstance(item, Tensor) and cat_dim is None:
                        ...
                        size = item.size(cat_dim) # type: ignore
                        device = item.device

                    batch[key].append(item)  # Append item to the attribute list.

                    slices[key].append(size + slices[key][-1]) # type: ignore
                    inc = data.__inc__(key, item)
                    if isinstance(inc, (tuple, list)):
                        inc = torch.tensor(inc)
                    cumsum[key].append(inc + cumsum[key][-1]) # type: ignore
